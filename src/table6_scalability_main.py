# -*- coding: utf-8 -*-
"""
Medium- and large-scale benchmark for the UAV inspection full-path MILP and
route-intensified ALNS-Q-learning-SA algorithm.

Revised experimental scales:
    Medium: M60, M70, and M80;
    Large : L100, L120, and L150.

Experimental logic:
    - Gurobi is run for M60, M70, M80, and L100 as a time-limited full-path
      MILP benchmark. L120 and L150 are used for scalability and stability.
    - ALNS-QSA is independently run 10 times for every instance under a strict
      nominal 600 s wall-clock limit.
    - Medium-scale comparisons report Gurobi UB/LB/gap and ALNS best/mean/CV.
    ...
    - Gurobi node files, a soft memory limit, checkpointing, and resume logic
      are enabled to avoid losing long-running results.
"""
...
# ---------- 路径设置 ----------
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "results", "table6_scalability_main_M60_L150"))
...
PAPER_TABLE_CSV = os.path.join(RESULT_DIR, "paper_table_medium_large_scalability.csv")
PAPER_TABLE_XLSX = os.path.join(RESULT_DIR, "paper_table_medium_large_scalability.xlsx")
...
# ---------- 调试开关 ----------
# 当前版本顺序运行 M60、M70、M80、L100、L120 和 L150，每个实例ALNS独立运行10次。
# 若需要重新进入单实例调试，可把 DEBUG_MODE 改为 True，并设置 DEBUG_INSTANCE_IDS。
DEBUG_MODE = False
DEBUG_INSTANCE_IDS = {"M80"}
DEBUG_PRINT_SOLUTION_AUDIT = False

# 同时运行 medium 与 large；程序按 INSTANCE_SPECS 顺序执行。
ENABLED_SCALES = ["medium", "large"]


# ---------- 实例规模：中规模60–80，大规模100–150 ----------
OPEN_PLATFORMS_FIXED = 4  # default for medium-scale instances; each instance can override it
N_UAV_TYPES = 3
N_NO_FLY_ZONES = 2

INSTANCE_SPECS = [
    # Medium-scale: time-limited Gurobi benchmark + 10 ALNS-QSA runs.
    # The number of candidate/open platforms is kept fixed within this group
    # so the growth from 60 to 80 points primarily reflects routing complexity.
    {"instance_id": "M60", "scale": "medium", "n_points": 60,
     "n_candidate_platforms": 12, "open_platforms_fixed": 4,
     "n_hazard_sources": 6, "seed": 20260960, "run_gurobi": True},
    {"instance_id": "M70", "scale": "medium", "n_points": 70,
     "n_candidate_platforms": 12, "open_platforms_fixed": 4,
     "n_hazard_sources": 6, "seed": 20260970, "run_gurobi": True},
    {"instance_id": "M80", "scale": "medium", "n_points": 80,
     "n_candidate_platforms": 12, "open_platforms_fixed": 4,
     "n_hazard_sources": 6, "seed": 20260980, "run_gurobi": True},

    # Large-scale: L100 retains a time-limited Gurobi benchmark; L120 and L150
    # focus on ALNS scalability, feasibility, and stability.
    {"instance_id": "L100", "scale": "large", "n_points": 100,
     "n_candidate_platforms": 15, "open_platforms_fixed": 5,
     "n_hazard_sources": 8, "seed": 20261000, "run_gurobi": True},
    {"instance_id": "L120", "scale": "large", "n_points": 120,
     "n_candidate_platforms": 15, "open_platforms_fixed": 5,
     "n_hazard_sources": 8, "seed": 20261020, "run_gurobi": False},
    {"instance_id": "L150", "scale": "large", "n_points": 150,
     "n_candidate_platforms": 18, "open_platforms_fixed": 6,
     "n_hazard_sources": 10, "seed": 20261050, "run_gurobi": False},
]
...
def main() -> None:
    print("UAV benchmark: medium M60-M80 and large L100-L150")
