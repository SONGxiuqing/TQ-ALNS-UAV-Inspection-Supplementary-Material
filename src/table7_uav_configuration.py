# -*- coding: utf-8 -*-
"""
Section 5.3 - homogeneous vs heterogeneous UAV configurations
==========================================
Purpose
-------
This version implements the three modeling/experimental changes discussed with the user:
1) capital costs are converted to planning-horizon equivalent costs by a CRF;
2) the number of opened platforms is fixed by scale (3/7, 4/15, 14/18), while their locations are optimized;
3) lower-level route distance, mission time, energy, charging, safe return, and service time
   are explicitly decoded and included in the objective.

Four algorithms are compared on the SAME instance, objective, operators, and feasibility rules:
Greedy / ALNS-only / ALNS+SA / TQ-ALNS.

Important academic-use note
---------------------------
The capital lives, discount rate, charging power, route-time value, and task capability mix below
are CALIBRATION PARAMETERS. Replace them with values supported by your paper/data before final submission.
Do not tune them separately by algorithm.
"""
...
ROBUST_INSTANCE_SEEDS = list(range(1, 11))   # Section 5.3: 10 independent random instances
...
# ------------------------ Section 5.3 configuration experiment ------------------------
...
def repair_uav(inst, sol, rng):
    ...
    assign_all_points_by_distance(inst, s, only_unassigned=True)
    recompute_required_types(inst, s)
    # Increase fleet on the most loaded platform if allowed.
    opens = open_set(s)
    loads = sorted(
        opens,
        key=lambda i: sum(inst.req_freq[j] for j in range(inst.J) if s.assign_j.get(j,None)==i),
...
def repair_route_rebalance(inst, sol, rng):
    ...
    assign_all_points_by_distance(inst, s, only_unassigned=True)
    opens = open_set(s)
    # Move a sample of long-distance points to closer feasible platforms.
    js = list(range(inst.J))
    rng.shuffle(js)
    for j in js[:min(80,inst.J)]:
...
# ============================================================================
# 11. Section 5.3 - homogeneous vs heterogeneous UAV configurations
# ============================================================================

CONFIGURATIONS = [
    ("All-light", [0], "Type A only"),
    ("All-medium", [1], "Type B only"),
    ("All-heavy", [2], "Type C only"),
    ("Heterogeneous", [0,1,2], "Type A + Type B + Type C"),
]


def set_uav_configuration(inst, label, allowed_types, setting):
    inst.configuration_label = label
    inst.allowed_types = list(allowed_types)
    inst.uav_type_setting = setting
    return inst


def _mean(xs):
    xs=list(xs); return sum(xs)/float(len(xs)) if xs else None


def _sd(xs):
    xs=list(xs); return statistics.stdev(xs) if len(xs)>=2 else 0.0 if xs else None


def fleet_mix_string(sol):
    cnt=[0,0,0]
    for i in open_set(sol):
        k=int(sol.platform_type.get(i,0)); cnt[k]+=int(sol.platform_uav_count.get(i,0))
    return "A%d/B%d/C%d" % tuple(cnt)


def run_uav_configuration_experiment():
    raw=[]
    print("\n=== Section 5.3 - homogeneous vs heterogeneous UAV configurations ===")
    print("Medium exact-open: 4/15 | independent instances=%d | iterations=%d" %
          (CONFIG_RUNS,HEURISTIC_ITERS))
    n=min(CONFIG_RUNS,len(CONFIG_INSTANCE_SEEDS),len(CONFIG_SEARCH_SEEDS))
    for run_id in range(1,n+1):
        instance_seed=CONFIG_INSTANCE_SEEDS[run_id-1]
        search_seed=CONFIG_SEARCH_SEEDS[run_id-1]
        print("\n--- RUN %02d | instance_seed=%d | search_seed=%d ---" %
              (run_id,instance_seed,search_seed))
        for label,allowed,setting in CONFIGURATIONS:
            inst=build_instance(instance_seed)
            set_uav_configuration(inst,label,allowed,setting)
            trace_file=None
            if CONFIG_SAVE_Q_TRACE:
                trace_file="config_%s_seed%d_qtrace.csv"%(label.replace("-","_").lower(),search_seed)
            sol=tq_alns(inst,search_seed,HEURISTIC_ITERS,trace_file=trace_file)
            compute_solution_obj(inst,sol)
            raw.append({
                "Configuration":label,
                "UAV type setting":setting,
                "Run":run_id,
                "Instance seed":instance_seed,
                "Search seed":search_seed,
                "Objective":sol.obj,
                "Coverage (%)":100.0*sol.coverage_ratio,
                "Feasible":int(bool(sol.feasible)),
                "Open platforms":len(open_set(sol)),
                "UAVs":total_uavs(sol),
                "Fleet mix":fleet_mix_string(sol),
                "Route distance (km)":sol.total_route_distance_km,
                "Total route time (h)":sol.total_route_time_h,
                "Makespan (h)":sol.route_makespan_h,
                "Runtime (s)":sol.runtime_s,
            })
            print("%-13s obj=%.6f feasible=%s cov=%.2f uavs=%d mix=%s dist=%.2f makespan=%.2f time=%.2f" %
                  (label,sol.obj,sol.feasible,100.0*sol.coverage_ratio,total_uavs(sol),
                   fleet_mix_string(sol),sol.total_route_distance_km,sol.route_makespan_h,sol.runtime_s))

    summary=[]
    for label,allowed,setting in CONFIGURATIONS:
        rows=[r for r in raw if r["Configuration"]==label]
        fr=[r for r in rows if r["Feasible"]==1]
        objs=[r["Objective"] for r in fr]
        mean_obj=_mean(objs); sd=_sd(objs)
        cv=(100.0*sd/mean_obj) if (mean_obj not in (None,0) and sd is not None) else None
        summary.append({
            "Configuration":label,
            "UAV type setting":setting,
            "Runs":len(rows),
            "Valid runs":len(fr),
            "Mean objective (feasible runs)":mean_obj,
            "SD":sd,
            "CV (%)":cv,
            "Mean coverage (%)":_mean(r["Coverage (%)"] for r in rows),
            "Feasibility rate (%)":100.0*len(fr)/float(len(rows)) if rows else 0.0,
            "Mean UAVs":_mean(r["UAVs"] for r in fr) if fr else None,
            "Mean route distance (km)":_mean(r["Route distance (km)"] for r in fr) if fr else None,
            "Mean makespan (h)":_mean(r["Makespan (h)"] for r in fr) if fr else None,
            "Mean runtime (s)":_mean(r["Runtime (s)"] for r in rows),
        })

    base_dir=os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "results", "table7_uav_configuration")); os.makedirs(base_dir,exist_ok=True)
    raw_path=os.path.join(base_dir,"section5_3_uav_configuration_raw_10instances_unified.csv")
    sum_path=os.path.join(base_dir,"section5_3_uav_configuration_summary_unified.csv")
    ...
    print("\nTable. Homogeneous versus heterogeneous UAV configurations")
    print("Configuration | Mean objective | SD | CV (%) | Coverage (%) | Feasibility (%) | Mean UAVs | Mean distance | Mean makespan | Mean time")
    for r in summary:
        def ff(x,nd=3): return "N/A" if x is None else ("%%.%df"%nd)%x
        print("%s | %s | %s | %s | %s | %s | %s | %s | %s | %s" % (
            r["Configuration"],ff(r["Mean objective (feasible runs)"],6),ff(r["SD"],6),
            ff(r["CV (%)"],3),ff(r["Mean coverage (%)"],2),ff(r["Feasibility rate (%)"],2),
            ff(r["Mean UAVs"],2),ff(r["Mean route distance (km)"],2),
            ff(r["Mean makespan (h)"],2),ff(r["Mean runtime (s)"],2)))
    print("Saved:",raw_path); print("Saved:",sum_path)
    return raw,summary


def print_parameter_summary():
    inst=build_instance(CONFIG_INSTANCE_SEEDS[0])
    print("Medium exact-open setting: %d/%d | J=%d | hazards=%d | NFZ=%d" %
          (inst.MIN_OPEN,inst.I,inst.J,inst.Hz,inst.Z))
    print("Equivalent platform cost=%.6f wan/day | UAV equivalent=%s" %
          (inst.platform_equiv_wan,[round(x,6) for x in inst.drone_equiv_wan]))
    print("Coverage>=%.2f | horizon=%.1fh | beta=%.2f | UAV/platform=%d..%d | max total UAV=%d" %
          (inst.min_cover_ratio,inst.horizon_h,inst.beta,MIN_UAVS_PER_OPEN_PLATFORM,
           MAX_UAVS_PER_PLATFORM,MAX_TOTAL_UAVS))
    print("Same instance/search seed is used by all four configurations within each run.")


def main():
    print_parameter_summary()
    run_uav_configuration_experiment()
